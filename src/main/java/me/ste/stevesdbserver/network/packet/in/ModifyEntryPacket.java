...
public class ModifyEntryPacket extends PacketIn {
    ...
    public void handle(Connection connection) {
        if(DatabaseManager.getInstance().hasPermission(connection.getUsername(), "modify_entry", this.database, this.table)) {
            connection.sendPacket(new ModifyEntryResponsePacket(DatabaseManager.getInstance().doAction(this.database, this.table, true, table -> {
                if(table != null) {
            ...
            connection.sendPacket(new ModifyEntryResponsePacket(false));
        }
    }
}